...
controller.AddUser = async function (req, res) {
    if (!req.body.phoneNo || req.body.phoneNo.length !== 10) {
        return res.status(500).send({success: false, error: "Phone number should 10 characters"});
    }
    ...
    if ((await validateUserName(user.username)) === false) {
        return res.status(400).json({success: false, error: `That username already exists`});
    }
